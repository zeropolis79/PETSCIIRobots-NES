.include "VAR.BGTASKS.asm"

.segment BANK(BK_GENERAL)
BACKGROUND_TASKS:
	BBC	GAME_FLAG, F_REDRAW_FIELD, UNIT_AI
	LDA	BG_TIMER
	CMP	#0
	BNE	UNIT_AI
	CLRB	GAME_FLAG, F_REDRAW_FIELD
	JSR	DRAW_MAP_WINDOW
UNIT_AI:
	...
	RTS
AI000:	LDA	#1
	STA	BG_TIMER	;RESET BACKGROUND TIMER
	; LDX	#$FF
	LDA	#0
	STA	UNIT
...
AI003:	;Unit exists and timer has triggered
	...
	LDY	UNIT_TYPE,X
	CPY	#24			;MAX DIFFERENT UNIT TYPES IN CHART
	BLT	AI004			;CONTINUE IF LESS THAN
	JMP	CUSTOM_AI		;ELSE IT MIGHT BE A CUSTOM AI
AI004:	LDA	AI_ROUTINE_CHART_H,Y
	PHA
...
AI_ROUTINE_CHART_L:
	.BYTE <(AI_DUMMY_ROUTINE-1)	;UNIT TYPE 00	;non-existent unit
	.BYTE <(AI_DUMMY_ROUTINE-1)	;UNIT TYPE 01	;player unit - can't use
	.BYTE <(LEFT_RIGHT_DROID-1)	;UNIT TYPE 02
	.BYTE <(UP_DOWN_DROID-1)	;UNIT TYPE 03
	.BYTE <(HOVER_ATTACK-1)		;UNIT TYPE 04
	.BYTE <(WATER_DROID-1)		;UNIT TYPE 05
	.BYTE <(TIME_BOMB-1)		;UNIT TYPE 06
	.BYTE <(TRANSPORTER_PAD-1)	;UNIT TYPE 07
	.BYTE <(DEAD_ROBOT-1)		;UNIT TYPE 08
	.BYTE <(EVILBOT-1)		;UNIT TYPE 09
	.BYTE <(AI_DOOR-1)		;UNIT TYPE 10
	.BYTE <(SMALL_EXPLOSION-1)	;UNIT TYPE 11
	.BYTE <(PISTOL_FIRE_UP-1)	;UNIT TYPE 12
	.BYTE <(PISTOL_FIRE_DOWN-1)	;UNIT TYPE 13
	.BYTE <(PISTOL_FIRE_LEFT-1)	;UNIT TYPE 14
	.BYTE <(PISTOL_FIRE_RIGHT-1)	;UNIT TYPE 15
	.BYTE <(TRASH_COMPACTOR-1)	;UNIT TYPE 16
	.BYTE <(UP_DOWN_ROLLERBOT-1)	;UNIT TYPE 17
	.BYTE <(LEFT_RIGHT_ROLLERBOT-1)	;UNIT TYPE 18
	.BYTE <(ELEVATOR-1)		;UNIT TYPE 19
	.BYTE <(MAGNET-1)		;UNIT TYPE 20
	.BYTE <(MAGNETIZED_ROBOT-1)	;UNIT TYPE 21
	.BYTE <(WATER_RAFT_LR-1)	;UNIT TYPE 22
	.BYTE <(DEMATERIALIZE-1)	;UNIT TYPE 23
...
MAGN4:	;Collision with robot detected.
	...
	LDX	UNIT_FIND
	LDA	UNIT_TYPE,X
	STA	UNIT_C,X	;make backup of unit type
...
DEMA2:	LDA	#97
	STA	UNIT_TILE
	LDA	UNIT_B,X	;target X coordinates
	STA	UNIT_LOC_X
	LDA	UNIT_C,X	;target Y coordinates
	STA	UNIT_LOC_Y
	LDA	#7		;Normal transporter pad
	STA	UNIT_TYPE,X
	JSR	CALCULATE_AND_REDRAW
...
AI_DOOR:
	LDX	UNIT
	LDY	UNIT_B,X
	CPY	#06	;make sure number is in bounds
	BCS	DOORA
	LDA	AIDB_H,Y
	PHA
	LDA	AIDB_L,Y
	PHA
DOORJ:	RTS		; indirect JMP
DOORA:	JMP	AILP	;-SHOULD NEVER NEED TO HAPPEN

AIDB_L:
	.BYTE	<(DOOR_OPEN_A-1)
	.BYTE	<(DOOR_OPEN_B-1)
	.BYTE	<(DOOR_OPEN_FULL-1)
	.BYTE	<(DOOR_CLOSE_A-1)
	.BYTE	<(DOOR_CLOSE_B-1)
	.BYTE	<(DOOR_CLOSE_FULL-1)
AIDB_H:
	.BYTE	>(DOOR_OPEN_A-1)
	.BYTE	>(DOOR_OPEN_B-1)
	.BYTE	>(DOOR_OPEN_FULL-1)
...
ELEVATOR:
	...
	LDA	ELDB_L,Y
	PHA
ELEVJ:	RTS		; indirect JMP
ELEVA:	JMP	AILP	;-SHOULD NEVER NEED TO HAPPEN

ELDB_L:
	.BYTE	<(ELEV_OPEN_A-1)
	.BYTE	<(ELEV_OPEN_B-1)
	.BYTE	<(ELEV_OPEN_FULL-1)
	.BYTE	<(ELEV_CLOSE_A-1)
	.BYTE	<(ELEV_CLOSE_B-1)
	.BYTE	<(ELEV_CLOSE_FULL-1)
ELDB_H:
	.BYTE	>(ELEV_OPEN_A-1)
	.BYTE	>(ELEV_OPEN_B-1)
	.BYTE	>(ELEV_OPEN_FULL-1)
...
ELPN2:	;PLAYER DETECTED, START ELEVATOR PANEL
	LDADDR	PTR_0, MSG_ELEVATOR
	JSR	PRINT_INFO
	LDADDR	PTR_0, MSG_LEVELS
	JSR	PRINT_INFO
	JSR	ELEVATOR_SELECT
	RTS
...
;on the map.  You must first define MAP_X and MAP_Y.
...
CFH02:	;first compare horizontal position
	...
	JMP	CFH01
CFH03:	CLC
	ADC	UNIT_B,X	;add hidden unit width
	CMP	MAP_X
	BCC	CFH01
CFH05:	;now compare vertical position
	...
	JMP	CFH01
CFH06:	CLC
	ADC	UNIT_C,X	;add hidden unit HEIGHT
	CMP	MAP_Y
	BCC	CFH01
